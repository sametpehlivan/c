...
void b_sort(int dizi[],int n)
{
	...
	while(bayrak==0)
	{   
		/// bayrak ile eðer sýralýysa dizi n durumda anlamasýiçin
		bayrak=1;
		for( int i=0;i<n-1;i++)
		{
			...
			n--; 
			///son sýralanan eleman en büyük olduðu için dizi konrolu küçültürülerek gidilmesi için
		}
	}
	fark=clock()-start;
	//printf("%.2lf\n",(float)fark);
...
void m_sort(int dizi[],int ilk,int son)
{
	...
	if (ilk<son)
	{ 
		//dizi iki elamnlý küçük dizilere bölünüyor
		int orta=(ilk+son)/2;
