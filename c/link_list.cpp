//LÝNK LÝST ÝLE DOSYADAN OKUNAN ELEMANLARI BAÐLI LÝSTEYE SIRALI BÝÇÝMDE YERLEÞTÝRME,ARAMA,EKLEME,SÝLME
...
int ekle(akademisyen *veri){
   ...
   if(veri->unvanID < root->unvanID){    //ünvaný ilk elemanýn ünanýndan büyükse
     iter=root;
   	 Akademisyen *temp=(Akademisyen*)malloc(sizeof(Akademisyen));
   	 temp=veri;
   	 temp->sonraki=iter;
    ...
    if(root->sonraki==NULL && veri->unvanID >root->unvanID){   //tek eleman varsa ve tekelmandan ünvaný kücükse ve sonraki sýralamalarda listenin sonunu konrtrolunu kýsaltmak için
     Akademisyen *temp=(Akademisyen*)malloc(sizeof(Akademisyen));
   	 temp=veri;
    ...
    if(veri->unvanID==root->unvanID ){      //ilk elamana ünavaný eþitse yýla göre sýralama
    	iter=root;
	 ...
	 iter=root;
    while(iter->sonraki!=NULL && veri->unvanID > iter->sonraki->unvanID ){ //araya eklemek için kontrol
    	iter=iter->sonraki;
	} 
    if(iter->sonraki->unvanID == veri->unvanID ){     //aradaki herhangi bir elemana unvaný eþitse yerleþtirme
...
void gorevlendirme(int a){         //kaç kisi görevlendirilecekse toplam adetten çýkarýp fark kadar listeyi ilerletiyoruz daha sonra ise listenin sonunu görene kadar bastýrýyoruz
	iter=root;                        
	for(int i=0;i<sayac-a;i++){	   //böylece ünvana göre görev alacaklarý sýralýyoruz
		iter=iter->sonraki;
...
int sil (int data){
	 ...
	 Akademisyen *temp=(Akademisyen*)malloc(sizeof(Akademisyen));
	 iter=root;
	 if(root->sicilNo== data){            //ilk eleman kontrolu yapýyoruz böylece ilk elemaný kaybetmeden hafizadan siliyoruz
	 	temp=root;
		 root=root->sonraki;
	     iter=root;
	     free(temp);
...
while(!(feof(dosya))){   //elemanlarý sýralýyarak ekliyor
